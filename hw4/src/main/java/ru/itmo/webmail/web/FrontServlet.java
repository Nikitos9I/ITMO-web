...
public class FrontServlet extends HttpServlet {
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        Method method;
        method = findMethod(clazz, "before");
        invokeMethod(page, clazz, method, request, response, view);
        method = findMethod(clazz, route.getAction());
        invokeMethod(page, clazz, method, request, response, view);
        method = findMethod(clazz, "after");
        invokeMethod(page, clazz, method, request, response, view);
    ...
    private void invokeMethod(Object page,
                              Class<?> clazz,
                              Method method,
                              HttpServletRequest request,
                              HttpServletResponse response,
                              Map<String, Object> view) throws IOException, ServletException {
        List<Object> params = new ArrayList<>();

        for (Class<?> current : method.getParameterTypes()) {
            if (current.equals(Map.class)) params.add(view);
            if (current.equals(HttpServletRequest.class)) params.add(request);
        }
    ...
    private Method findMethod(Class<?> currentClass, String methodName) throws NotFoundException {
        Method method = null;

        for (Class<?> c = currentClass; method == null && c != null; c = c.getSuperclass()) {
    ...
    private Route findRoute(HttpServletRequest request) {
        ...
        StringBuffer className = new StringBuffer(BASE_PAGE_NAME);
